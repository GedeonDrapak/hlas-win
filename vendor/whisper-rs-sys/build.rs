...
fn main() {
    ...
    // CMake installs the static archives here. Without this explicit path,
    // Windows builds fail to find ggml even though the library compiled.
    let library_dir = destination.join("lib");
    println!("cargo:rustc-link-search=native={}", library_dir.display());
    // GNU rustc searches `lib{name}.a`, while whisper.cpp's CMake install uses
    // `{name}.a`. Make build-output aliases only for GNU Windows targets.
    if target.contains("windows-gnu") {
        for name in ["ggml", "ggml-base", "ggml-cpu"] {
            let source = library_dir.join(format!("{name}.a"));
            let alias = library_dir.join(format!("lib{name}.a"));
            if source.is_file() && !alias.exists() {
                std::fs::copy(source, alias).expect("failed to create GNU library alias");
            }
        }
    }
    if cfg!(feature = "intel-sycl") {
